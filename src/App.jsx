...
export default function App() {
  ...
  useEffect(() => {
    ...
    function drawLoop() {
      ...
      if (c && v) {
        ...
        ctx.fillText('DEBUG', 15, 28)

        ctx.font = '12px monospace'
        ctx.fillText(`Mic RMS: ${micRMS.toFixed(3)}`, 15, 45)
        ...
        ctx.fillText(`Status: ${status}`, 15, 105)
        ctx.fillText(`Scanning: ${scanning ? 'YES' : 'NO'}`, 15, 120)
      }
  ...
  async function startScan() {
    ...
    try {
      // Request with raw audio constraints (no processing)
      const constraints = {
        audio: {
  ...
  return (
    <div className="fixed inset-0 bg-black flex flex-col">
      <video ref={videoRef} className="flex-1 w-full h-full object-cover" />
      ...
      <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex gap-3">
        <button
          onClick={startScan}
          disabled={scanning}
          className="px-6 py-3 bg-green-600 hover:bg-green-700 disabled:bg-gray-600 rounded-lg text-white font-semibold"
        >
        ...
        <button
          onClick={stopScan}
          disabled={!scanning}
          className="px-6 py-3 bg-red-600 hover:bg-red-700 disabled:bg-gray-600 rounded-lg text-white font-semibold"
        >
          Stop
        </button>
      </div>
